...
class PhpReports {
	...
	public static function init($config = null) {
		...
		if($config == null) {
			$config = config_path('phpreports.php');
			if($config == null){
		...
		if(!file_exists(__DIR__.'/../../config/config.php.sample')) {
			throw new Exception("Cannot find sample config. Please leave config/config.php.sample in place for default values.");
		}

		$default_config = include(__DIR__.'/../../config/config.php.sample');
		$config = include($config);

		self::$config = array_merge($default_config, $config);

		//NEEDED LATER
		//var_dump(str_replace(array('\\',' '), array('/','%20'), dirname($_SERVER['SCRIPT_NAME'])));
		//die();

		self::$request = Flight::request();
		$path = self::$request->base;

		if ((Request::server('HTTPS') !== null) && (Request::server('HTTPS') == 'on' || Request::server('HTTPS') == 1) || (Request::server('HTTP_X_FORWARDED_PROTO') !== null) && Request::server('HTTP_X_FORWARDED_PROTO') == 'https') {
			$protocol = 'https://';
		} else {
			$protocol = 'http://';
		}
		self::$request->base = $protocol.rtrim(Request::server('HTTP_HOST').self::$request->base,'/');
		...
		$base_template_path = is_dir(resource_path('views/vendor/phpreports')) ? resource_path('views/vendor/phpreports') : __DIR__.'/../../templates';
		$template_dirs = array($base_template_path.'/default',$base_template_path);
		if(file_exists($base_template_path.'/local')) array_unshift($template_dirs, $base_template_path.'/local');
	...
	protected static function getReports($dir, &$errors = null) {
		...
		foreach($reports as $key=>$report) {
			...
			if(is_dir($report)) {
				...
				$extexplode = explode('.',$report);
				$ext = array_pop($extexplode);
